          ...
          comments_tree.each do |comment|
            if comment.parent_id == parent_id
              ...
              if  k != 25 && User.current.allowed_to?({:controller => 'wiki_extensions', :action => 'reply_comment'}, @project)
                ret << link_to_function(l(:button_reply), "$('##{form_reply_id}').show();", :class => 'icon icon-comment')
              end

              # Edit link
              if User.current.allowed_to?({:controller => 'wiki_extensions', :action => 'update_comment'}, @project) or User.current.id == comment.user.id or User.current.admin
                ret << link_to_function(l(:button_edit), "$('##{div_comment_id}').hide();$('##{form_div_id}').show();$('##{form_reply_id}').hide();",:class => 'icon icon-edit wiki_font_size')
              end

              # Delete link
              if User.current.allowed_to?({:controller => 'wiki_extensions', :action => 'destroy_comment'}, @project) or User.current.admin
                del_link = link_to_if_authorized(l(:button_delete), {:controller => 'wiki_extensions', :action => 'destroy_comment', :id => @project, :comment_id => comment.id}, :class => "icon icon-del", :confirm => l(:text_are_you_sure))
              ...
              ret << "\n"
              ret << l(:label_added_time_by, :author => link_to_user(comment.user), :age => time_tag(comment.updated_at))
              ...
              ret << text_area_tag(:reply,'', :rows => 5, :cols => 70, :id => textarea_id, :accesskey => accesskey(:edit), :class => 'wiki-edit')
              ret << '<br/>'
